...
class NNPUHostDeviceSplitter : public IRMutator {
 public:
  // IR for NNPU has no Allocate Node.
  // Stmt Mutate_(const Allocate* op, const Stmt& s) final {
 ...
 private:
  Stmt SplitDeviceFunc(Stmt body) {
    ...
    NNPUIRUseDefAnalysis m;
    // m.visit_coproc_scope = false;
...
Array<Var> NNPUUndefinedVars(const Stmt& stmt, const Array<Var>& args) {
  NNPUIRUseDefAnalysis m;
  for (Var arg : args) {
...
Array<LoweredFunc> NNPUSplitHostDevice(LoweredFunc func) {
  return NNPUHostDeviceSplitter().Split(func);
}
