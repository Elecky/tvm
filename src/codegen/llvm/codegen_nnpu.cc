...
llvm::Value* CodeGenNNPU::VisitExpr_(const Call *op) {
  if (op->call_type == Call::Intrinsic && 
      op->is_intrinsic("llvm_intrin_with_side_effect")) {
...
runtime::Module BuildNNPU(Array<LoweredFunc> funcs, std::string target) {
  ...
  std::cout << ll;
  exit(0);  // instant exit to see the generated IR.
  ...
  return NNPUModuleCreate(ptx, ExtractFuncInfo(funcs), ll);
}
