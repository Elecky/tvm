...
def test():
    ...
    gtm_buf = tvm.compute((16, ), lambda i: tvm.sum(a_buf[k2, i], axis=k2), 'gtm_buf')
    sph.MarkScope(gtm_buf)
    gtm_host, gtm_dram = nnpu.utils.CopyBufToH(gtm_buf, 'gtm', sph)

    s = tvm.create_schedule([add_host.op,mul_host.op,gtm_host.op])
    sph.Transform(s)
    ...
    print(nnpu.lower(s, [a, add_host,mul_host,gtm_host], simple_mode=True))

    func = nnpu.build(s, [a, add_host,mul_host,gtm_host], 'nnpu', 'llvm', name='nnpu_func')
    #exit()
