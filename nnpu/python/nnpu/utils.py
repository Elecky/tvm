...
def DMACopyHtoDram(tensor, name_prefix, sph):
    env = get_env()
...
def CopyHtoBuf(tensor, name_prefix, sph, dst_scope='uni'):
    env = get_env()
...
def CopyBufToDram(tensor, name_prefix, sph):
    env = get_env()
    tensor_dram = tvm.compute(tensor.shape, lambda *i: tensor(*i), name_prefix + "_dram")
...
def CopyBufToH(tensor, name_prefix, sph):
    env = get_env()
