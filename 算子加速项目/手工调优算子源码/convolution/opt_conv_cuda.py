...
# The  origin sizes of inputs and filters
batch = 256
in_channel = 256
out_channel = 512
in_size = 14
...
print(tvm.lower(s,[A,W,B],simple_mode=False))
...
time=evaluator(a, w, b).mean
#print('%f GFlops'%(float(28*28*64*128)/time/1e9))
#print('%f GFlops'%(float(out_size*out_size*in_channel*out_channel*kernel*kernel*2)/time/1e9))
#print('%f GFlops'%(float(14*14*256*512*3*3*2*256)/time/1e9))
