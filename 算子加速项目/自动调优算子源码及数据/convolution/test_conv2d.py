...
def tune_tasks(tasks,
    ...
    for i, tsk in enumerate(reversed(tasks)):
        ...
        if tuner == 'xgb' or tuner == 'xgb-rank':
            ...
            tuner_obj = HCTuner(tsk)
        elif tuner == 'lgb':
            tuner_obj = LGBTuner(tsk)
        elif tuner=='bs':
            tuner_obj=BSTuner(tsk)
        else:
...
# extract workloads from relay program
    ...
    net = relay.Function([data,weight], z)
    print(net.astext())
    #net, params = tvm.relay.testing.init.create_workload(net)
    net=relay.Module.from_expr(net)
    params={
        'weight':tvm.nd.array(np.random.rand(512,256,1,1).astype(np.float32))
    }
    tasks = autotvm.task.extract_from_program(net["main"], target = target,
